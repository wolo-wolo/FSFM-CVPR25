...
class feature_generator(nn.Module):
    ...
    def __init__(self, args):
        ...
        if args.pt_model is None:
            ...
            self.vit = models_vit_fs_adapter.__dict__[args.model](
            ...
            if global_pool:
                assert set(msg.missing_keys) == {'head.weight', 'head.bias', 'fc_norm.weight', 'fc_norm.bias',
                                                 'adapter.linear1.weight', 'adapter.linear2.weight',
    ...
    def forward(self, input):
        ...
        x_adapter, x_bottleneck = self.vit.adapter(feat)
        proj_features = self.vit.projector(x_bottleneck)

        feat = torch.concatenate([feat, x_adapter], dim=-1)

        return feat, F.normalize(proj_features, dim=-1)
...
class feature_embedder(nn.Module):
    ...
    def __init__(self, args):
        ...
        # Adjust input_dim to account for concatenation in feature_generator
        input_dim *= 2  # Concatenation doubles(VIT and FS-Adapter) the dimension
...
class fas_model_fix(nn.Module):
    ...
    def forward(self, input, norm_flag=True):
        feature, adapter_features = self.backbone(input)
        feature = self.embedder(feature, norm_flag)
        classifier_out = self.classifier(feature, norm_flag)
        return classifier_out, feature, adapter_features
